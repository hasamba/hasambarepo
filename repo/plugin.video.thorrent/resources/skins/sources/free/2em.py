...
import re
from resources.modules.pen_addons import download_file,unzip,decode_nom
...
try:
    ...
    dataDir = xbmc.translatePath(xbmcaddon.Addon().getAddonInfo("profile"))
...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    name=name.replace("'","")
    l_list='https://kcat123.github.io/FTV/Public/FlashTV.txt'
    ...
    for index,name,f_link,icon,fanart,plot,data,date,year,genre,father,type in match:
        # import logging
        # log.warning('Deoding33 '+str(match))
        progress='Links-'+str(match)
        count+=1
        if stop_all==1:
            break
        ...
        if '$$$' in f_link:
            ...
            for link_in in f_link_s:
                ...
                sys.path.append( path)
                import resolveurl
                link2 =resolveurl .HostedMediaFile (url =f_link_s ).resolve ()
                ...
                all_links.append((nm,'Direct_link$$$resolveurl'+link_in,str(size),res))
            ...
            sys.path.append( path)
            import resolveurl
            link2 =resolveurl .HostedMediaFile (url =f_link ).resolve ()
            ...
            if tv_movie=='movie':
                ...
                nm=clean_name(original_title,1)+'.S%sE%s'%(season_n,episode_n)
            all_links.append((nm,'Direct_link$$$resolveurl'+f_link,str(size),res))
            global_var=all_links
