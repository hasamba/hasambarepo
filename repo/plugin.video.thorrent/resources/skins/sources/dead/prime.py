...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    x,cook=get_html("https://pw.unblockit.uno/?s=%s&t=y&m=m&w=q"%(imdb_id),headers=base_header,get_cookies=True,timeout=10).content()
    ...
    #time.sleep(1)
    y=get_html('https://pw.unblockit.uno'+m,headers=headers,cookies=cook).content()
    logging.warning('https://pw.unblockit.uno'+m)
    if tv_movie=='tv':
        ...
        for items in m_pre:
            ...
            if 'season-%s-episode-%s$$$'%(season,episode) in m2:
                    #time.sleep(1)
                    y=get_html('https://pw.unblockit.uno'+m2.replace('$$$',''),headers=headers,cookies=cook).content()
            ...
            m3=re.compile(regex).findall(y)[0]
            # import xbmc
            # xbmc.executebuiltin((u'Notification(%s,%s)' % ('Kodi Anonymous', m3)))
