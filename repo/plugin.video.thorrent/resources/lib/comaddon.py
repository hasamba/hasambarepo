...
class addon():
    ...
    def VSlang(self, lang):
        return xbmc_tranlate_path(xbmcaddon.Addon(self.addonId).getLocalizedString(lang)) if self.addonId else xbmc_tranlate_path(ADDONVS.getLocalizedString(lang))
        #Bug avec accent xbmc_tranlate_path(xbmcaddon.Addon('plugin.video.thorrent').getLocalizedString(lang)).decode('utf-8')
...
def VSPath(pathSpecial):
    if xbmc.getInfoLabel('system.buildversion')[0:2] >= '19':
        ...
        path = xbmc_tranlate_path(pathSpecial)
    return path
