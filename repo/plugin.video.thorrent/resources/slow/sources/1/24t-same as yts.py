...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    for page in range(1,4):
        try:
            x=requests.get(('https://24tor.com/v2/?word=%s&page=%s'%(search_url,str(page))).replace(' ','%20'),headers=base_header,timeout=10).content.decode('utf-8')
        except:
        ...
        for itm in macth_pre:
            ...
            for hash,title,size in macth_pre2:
                     ...
                     try:
                        y=requests.get('https://24tor.com/v2/'+hash,headers=base_header,timeout=10).content.decode('utf-8')
                     except:
