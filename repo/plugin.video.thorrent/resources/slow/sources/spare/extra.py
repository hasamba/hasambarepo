...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    regex='a href="magnet(.+?)".+?td class="tli".+?title="(.+?)".+?td class="sn">(.+?)<.+?td class="ln">(.+?)<'
      ...
      for page in range(1,4):
        ...
        for items in macth_pre:
            ...
            for link,title,seed,peer in match:
                     ...
                     try:
                         o_size=size
                         size=float(o_size.replace('GB','').replace('GiB','').replace('MB','').replace(",",'').replace("MiB",''))
                         if 'MB' in o_size:
                           size=size/1000
                     ...
                     if int(Addon.getSetting('min_seed'))>int(seed):
                        continue
                     if size<max_size:
                       all_links.append((title,'magnet'+o_link,str(size),'[COLOR deepskyblue]Peer:[/COLOR] %s [COLOR deepskyblue]Seed:[/COLOR] %s'%(peer,seed),res))
