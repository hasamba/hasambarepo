...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    regex='<div class="resultdivbottonlength">(.+?)<.+?<div class="resultdivbottonseed">(.+?)<.+?class="hideinfohash">(.+?)<.+?class="hideinfohash">(.+?)<'
    ...
    for page in range(1,4):
      for itt in search_url:
        ...
        for items in m_pre:
            ...
            for size,seed,hash,nam in match:
                if f_seeds:
                        seed=seed.replace(",","")
                ...
                if int(size)<max_size:
                   ...
                   all_links.append((seed_t+nam,lk,str(size),res))
