...
import urllib,logging,base64,json,xbmc
...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    uri='https://soap2dayto.org/search/keyword/%s/'%clean_name(original_title,1).replace(' ','%20')
    ...
    for items in m_pre:
        ...
        for yr,lk,nm in m:
            ...
            if show_original_year in yr and clean_name(original_title,1).lower()==nm.replace(':','%3a').lower():
                ...
                x=get_html('https://soap2dayto.org'+lk,headers=cook[1],cookies=cook[0]).content()
                y=x
                ...
                if tv_movie=='tv':
                    ...
                    y=get_html('https://soap2dayto.org'+llk,headers=cook[1],cookies=cook[0]).content()
               
                regex='type="hidden" id="hId" value="(.+?)"'
                did=re.compile(regex).findall(y)[0]
                
                cook[1]['Referer']= 'https://soap2dayto.org'+lk
                data = {
                  'pass': did
                }
                ...
                response=get_html('https://soap2dayto.org/home/index/'+added, headers=cook[1],cookies=cook[0], data=data).content()
