...
def tv_show_menu():
    ...
    aa=addDir3(Addon.getLocalizedString(32013),'https://api.themoviedb.org/3/tv/on_the_air?api_key=b370b60447737762ca38457bd77579b3&language=he-US&region=US&with_original_language=en&page=1',14,BASE_LOGO+'on_air.png',all_fanarts['32013'],'TMDB')
    all_d.append(aa)
    
    
    aa=addDir3(Addon.getLocalizedString(32014),'https://api.themoviedb.org/3/discover/tv?api_key=b370b60447737762ca38457bd77579b3&language={0}&sort_by=popularity.desc&first_air_date_year='+str(now.year)+'&with_original_language={1}&language=he&page=1'.format(lang,'en'),14,BASE_LOGO+'int.png',all_fanarts['32014'],'New Tv shows')
    all_d.append(aa)
    #new episodes
...
def search_history(url,icon,fan):
    ...
    dbcur.execute("SELECT * FROM search_string2 ORDER BY rowid DESC")
...
def refresh_list(user_params,sys_arg_1_data,Addon_id=""):
    ...
    if mode==None :
        ...
        get_movies('http://api.themoviedb.org/3/search/movie?api_key=b370b60447737762ca38457bd77579b3&query={0}&language={1}&append_to_response=origin_country&page=1'.format(search_entered,lang),global_s=True)
        
        addNolink( '[COLOR blue][I]%s[/I][/COLOR]'%Addon.getLocalizedString(32099), id,27,False,fanart=' ', iconimage=' ',plot=' ')
        get_movies('http://api.themoviedb.org/3/search/tv?api_key=b370b60447737762ca38457bd77579b3&query={0}&language={1}&page=1'.format(search_entered,lang),global_s=True)
       ...
       if len(all_folders)>0:
        ...
        from resources.default import populate_playlist
        ...
        addNolink( '[COLOR blue][I]---%s---[/I][/COLOR]'%Addon.getLocalizedString(32024), id,27,False,fanart=' ', iconimage=' ',plot=' ')
        url=url.replace('%20',' ')
        get_movies('http://api.themoviedb.org/3/search/movie?api_key=b370b60447737762ca38457bd77579b3&query={0}&language={1}&append_to_response=origin_country&page=1'.format(que(url),lang),global_s=True)
        
        addNolink( '[COLOR blue][I]%s[/I][/COLOR]'%Addon.getLocalizedString(32099), id,27,False,fanart=' ', iconimage=' ',plot=' ')
        get_movies('http://api.themoviedb.org/3/search/tv?api_key=b370b60447737762ca38457bd77579b3&query={0}&language={1}&page=1'.format(que(url),lang),global_s=True)
        addNolink( '[COLOR blue][I]TVDB[/I][/COLOR]', id,27,False,fanart=' ', iconimage=' ',plot=' ')
        ...
        from resources.default import sex
        sex()
    elif mode==202:
        from resources.default import populate_playlist
        ...
        if keyboard.isConfirmed() :
               search_entered = (keyboard.getText().replace("'","%27"))
               if search_entered!='':
                  from resources.default import search_result_movie
                  search_result_movie(search_entered)
    elif mode==213:
        from resources.default import iptv
        iptv()
    elif mode==214:
        
            DIALOG         = xbmcgui.Dialog()
            choice = DIALOG.yesno('Thorrent', "האם להתקין את התוסף Elementum?", yeslabel="[B][COLOR WHITE]אישור[/COLOR][/B]", nolabel="[B][COLOR white]ביטול[/COLOR][/B]")
            if choice == 1:
              from resources.default import elementum
              elementum()
            else:
                   sys.exit()
    elif mode==215:
            ...
            DIALOG         = xbmcgui.Dialog()
            choice = DIALOG.yesno('Thorrent', "האם להתקין את התוסף Anonymous Torrents?", yeslabel="[B][COLOR WHITE]אישור[/COLOR][/B]", nolabel="[B][COLOR white]ביטול[/COLOR][/B]")
            if choice == 1:
              from resources.default import torrest
              torrest()
            else:
                   sys.exit()
    match=[]
