...
def getMediaLinkForGuest_new(url):
    global file
    oParser = cParser()        
    ...
    sHtmlContent1 = requests.get(url,headers=headers).content
    ...
    if 'nonymous' in code:
      print 'ANONONONONONONONON'
    
    regex='<p id=".+?" style=.+?>(.+?)<'

    page_code=re.compile(regex).findall(sHtmlContent1)
  
    if len( page_code)==0:
                        
            sPattern = 'id="%s">(.+?)<'%Item_url.replace('#','')
            logging.warning(sPattern)
            page_code=re.compile(sPattern).findall(sHtmlContent1)[0]
    '''
    #interpreteur JS
    JP = JsParser()
    Liste_var = []
    ...
    regex2="case'7':(.+?);continue;"
    match_r1=re.compile(regex2).findall(code)[0]
    
    regex3="_0x531f91=(.+?);continue;"
    match_r2=re.compile(regex3).findall(code)[0]
    if 'window.cc_cc_cc' in code:
        regex3="window.cc_cc_cc=(.+?);"
        match_r3=re.compile(regex3).findall(code)[0]
        fixed_code=code.replace(match_r2,'"'+page_code+'"').replace(match_r1,'"640K ought to be enough for anybody"').replace("window.cc_cc_cc=%s;"%match_r3,'')+'var doc=_0x1bf6e5;doc=_0x1bf6e5;'
    else:
         logging.warning(match_r2)
         logging.warning(page_code)
         regex3="window.ffff=(.+?);"
         match_r3=re.compile(regex3).findall(code)[0]
         fixed_code=code.replace(match_r2,'"'+page_code+'"').replace(match_r1,'"640K ought to be enough for anybody"').replace("window.ffff=%s;"%match_r3,'')+'var doc=_0x1bf6e5;doc=_0x1bf6e5;'
        
    from js2py.internals import seval
   
    fixed_code="var document={'write':'ok'};"+fixed_code
    
   
    result=seval.eval_js_vm(fixed_code)
    logging.warning( 'final')
    #logging.warning('var ffff="'+Item_url.replace('#','')+'";'+fixed_code)
    logging.warning( result)
    final="https://openload.co" + "/stream/" + result + "?mime=true" 
    return  final
    ...
    print 'start3'
    if not(url):
        logging.warning('Rate, debug: ' + str(Liste_var))
        return False,False
