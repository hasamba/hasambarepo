...
def get_links(tv_movie,original_title,season_n,episode_n,season,episode,show_original_year,id):
    ...
    if tv_movie=='movie':
        lk='https://ext.torrentbay.to/search/%s/%s/?c=movies'
        search_url=[((clean_name(original_title,1).replace(' ','%20')+'%20'+show_original_year)).lower()]
    else:
      lk='https://ext.torrentbay.to/search/%s/%s/?c=tv'
    ...
    for page in range(1,4):
     for itt in search_url:
        ...
        for items in m:
            regex='a href=".+?"><b>(.+?)</b>.+?href="magnet(.+?)".+?Size</span>.+?<span>(.+?)</span>'
            m2=re.compile(regex,re.DOTALL).findall(items)
            for nm,lk,size in m2:
