...
def get_html(url,headers,data,method,data2={}):
    ...
    handlers = [url_lib.HTTPCookieProcessor(cookjar),url_lib.HTTPHandler(), url_lib.HTTPSHandler()]
                   
    opener = url_lib.build_opener(*handlers)
   
   
    if len(data2)>0:
        request = url_lib.Request(url,  headers=headers,data=urllib.urlencode(data2))
    elif len(data)>0:
       
        request = url_lib.Request(url,  headers=headers,data=json.dumps(data).encode("utf-8"))
    else:
        request = url_lib.Request(url,  headers=headers)
