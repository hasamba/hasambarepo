...
try:
    ...
    user_dataDir=os.path.join(dataDir,'cache_f','ornm')

except Exception as e:
   
    dir_path = os.path.dirname(os.path.realpath(__file__))
    mypath=os.path.join(dir_path,'..\done')
    user_dataDir=os.path.join(mypath,'cache_f','ornm')
    ...
    for index,name,f_link,icon,fanart,plot,data,date,year,genre,father,type in match:
        ...
        if '$$$' in f_link:
            ...
            for link_in in f_link_s:
                ...
                if len(q)>0:
                    res=str(q[0])
                all_links.append((original_title,'HEBSOURCE$$$'+link_in,str(size),res))
            ...
            if len(q)>0:
                res=str(q[0])
            all_links.append((original_title,'HEBSOURCE$$$'+f_link,str(size),res))
            global_var=all_links
