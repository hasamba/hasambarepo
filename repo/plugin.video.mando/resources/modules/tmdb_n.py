...
from  resources.modules.public import addNolink,addDir3,addLink,lang,user_dataDir,meta_get
import json,xbmc
...
class tmdb:
    ...
    def local_cache(self,function,timeout,url):
        ...
        if cachedata:
            all_results,all_ids,main_data,saved_time=eval(cachedata)
            ...
            if update == True:
                ...
                log.warning('Update True')
                t=time.time()
                all_results,all_ids,main_data=function(url)
                c= (json.dumps(all_results), json.dumps(all_ids), json.dumps(main_data),t)
                c = set_property(url,repr(c))
                
                
                
        
        if not cachedata or all_ids==[] or all_ids=={}:
            ...
            t = int(time.time())
            if match!=None:
                all_results,all_ids,main_data=json.loads(match[1]),json.loads(match[2]),json.loads(match[3])
                ...
                if update == True or all_ids=={}:
                    log.warning('Update Now')
                    all_results,all_ids,main_data=function(url)
                    
                    cachedata = (json.dumps(all_results), json.dumps(all_ids), json.dumps(main_data),t)
                    cachedata = self.set_property(url,repr(cachedata))
                    
                    
                    dbcur.execute("INSERT INTO local_cache Values (?, ?, ?, ?, ?, '')" , (url, json.dumps(all_results), json.dumps(all_ids), json.dumps(main_data),t))
                    dbcon.commit()
                dbcur.close()
                dbcon.close()
                return all_results,all_ids,main_data
    ...
    def is_in_english(self,term,title):
        try:
            import unicodedata
            english=False
            ...
            for i in term:
                    ...
                    char_type=unicodedata.name(i.strip())
                    if 'LATIN' in char_type:
                        english=True
                        #log.warning('English True:%s, %s,%s ,%s '%(term,i,unicodedata.name(i.strip()),title))
                        break
                    else:
                        #log.warning('English NOT:%s, %s,%s ,%s'%(term,i,unicodedata.name(i.strip()),title))
                        break
            return english
        except Exception as e:
            log.warning('Error in is_English:'+str(e))
            return False
    ...
    def thread_all_data(self,url):
        log.warning('TRD all data')
        if self.all_ids=={}:
            log.warning('TRD Empty')
            self.all_results,self.all_ids,self.main_data=self.local_cache(self.build_data,0,self.url)
        ...
        if Addon.getSetting("extra_data")=='true':
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for id in self.all_ids:
                    ...
                    i+=1
                wait(futures)
        return self.all_d,self.all_ids,self.main_data
    ...
    def add_dir(self,season=0):
        ...
        if self.simple_data==False:
            self.all_d,self.all_ids,self.main_data=self.local_cache(self.thread_all_data,24,'data'+self.url)
        ...
        for ids in self.all_d:
            ...
            if KODI_VERSION>19:
                ...
                info_tag.setPremiered(str(meta_get(video_data,'premiered')))
        ...
        if not self.simple_data:
            ...
            listitem=xbmcgui.ListItem(offscreen=True)
            listitem.setLabel('[COLOR aqua][I]בחר עמוד[/I][/COLOR]')
            
            
            if KODI_VERSION>19:
                info_tag = listitem.getVideoInfoTag()
                
                info_tag.setTitle('[COLOR aqua][I]בחר עמוד[/I][/COLOR]')
            else:
                listitem.setInfo('Video', infoLabels={'title':'[COLOR aqua][I]בחר עמוד[/I][/COLOR]'})
            ...
            params['description']=''
            params['url']=link+'page='+str(int(page_no)+1)
            params['name']='[COLOR aqua][I]בחר עמוד[/I][/COLOR]'
           
            params['heb_name']='[COLOR aqua][I]בחר עמוד[/I][/COLOR]'
            
            params['mode']=204
           
            
            params['eng_name']='[COLOR aqua][I]בחר עמוד[/I][/COLOR]'
