...
def get_links(tv_movie,original_title,name,season_n,episode_n,season,episode,show_original_year,id,check_one=False):
    ...
    try:
        ...
        if check_one==True:
            ...
            global_var=global_var+GetJson(imdbid,'1',season=season.replace('%20','0'),episode=episode.replace('%20','0'),version=lowercase_with_underscores(item['title']),check_one=check_one,global_var=global_var)
        log.warning('len(global_var2)')
        global_var=set(global_var)
