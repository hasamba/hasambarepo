...
class Items:
    def __init__(self, addon, addon_base, settings, saved_newsgroups, vfs):
        self.addon = addon
        self.addon_base = addon_base
        self.settings = settings
        self.saved_newsgroups = saved_newsgroups
        self.vfs = vfs
    ...
    def search(self):
        ...
        # Saved newsgroups (same store as Browse → Saved Newsgroups)
        history = self.saved_newsgroups.get()
        for k in sorted(list(history), reverse=True):
            query = history[k].get("newsgroup")
            if not query:
                continue
